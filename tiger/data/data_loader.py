...
class GraphCollator:
    ...
    def collate_memory_nodes(self, nids: np.ndarray, ts: np.ndarray) -> List[Tuple]:
        ...
        layers[0] = [nids, None, None, None, None]  # dummy layer storing input node ids
        unique_ids = set()
        for depth in range(len(layers)):
            ...
            if depth == 0:
                layers[depth] = (torch.from_numpy(neigh_nids).long(), None, None, None, None)
            else:
                layers[depth] = (
    ...
    def _collate_memory_nodes_recursive(self, nids: np.ndarray, ts: np.ndarray,
        ...
        if depth == 0:
            return 
        neigh_nids, neigh_eids, neigh_ts, _, neigh_re, neigh_degree = self.graph.sample_temporal_neighbor(
...
def load_jodie_data(
    ...
    return (nfeats, efeats, shift_max, full_data, train_data, val_data, test_data,
...
def compute_delta_max(srcs: np.ndarray, dsts: np.ndarray, ts: np.ndarray
                     ) -> float:
    """
